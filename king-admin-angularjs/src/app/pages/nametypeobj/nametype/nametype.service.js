(function() {
    ...
    angular.module('KingAdmin.pages.nametypeobj.nametype')
        .factory('NameTypeService', NameTypeService);

    /** @ngInject */
    function NameTypeService($resource, toastr, CommonService) {

        var rest = $resource('api/name_type/:id', {}, {
            'create': { method: 'POST' },
            'update': { method: 'PUT' },
        });

        function getSmartData(param, callback) {
            $resource('api/name_type/getSmartData', {}, {
                'query': { method: 'POST' }
            }).query(param,
                function(data) {
                    console.log(data);
